...
const api1= async (req,res) => {
  ...
  const [rows] = await connection.query(`select distinct a.RATE_AMT from t_exchange_rate a where a.STORE_CD = "${STORE_CD}" and a.EX_TO like "%${EX_TO}%" and "${DATE}" between a.start_dt and a.end_dt;`);
...
const api1_2 = async (req,res) =>{
  ...
  const DATE = req.query.DATE;
    ...
    const rows = await connection.query(`select distinct a.PROD_CD, b.PROD_NAME, a.PACKING_SIZE, a.PROD_RETAIL, a.PROD_RETAIL*TRUNCATE(c.RATE_AMT, 0) as product_retail_thb, 
